...
if ($page == 'callback') {
    ...
    if ($resultCode == 0) {
        ...
        foreach ($callbackMetadata->Item as $item) {
            ...
            elseif ($item->Name == 'MpesaReceiptNumber') $mpesaReceiptNumber = $item->Value;
            elseif ($item->Name == 'TransactionDate') $transactionDate = $item->Value;
            elseif ($item->Name == 'PhoneNumber') $phoneNumber = $item->Value;
        }
...
if ($page == 'process_eligibility' && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Basic Validation
    $errors = [];
    if (empty($full_name) || strlen($full_name) < 3) $errors[] = "A valid full name is required.";
    if (empty($id_number) || !is_numeric($id_number) || strlen($id_number) < 7) $errors[] = "A valid ID number is required.";
    if (empty($loan_type)) $errors[] = "Please select a loan type.";
...
if ($page == 'process_payment' && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $environment = getenv('MPESA_ENVIRONMENT'); // 'sandbox' or 'live'
    ...
    if(curl_errno($ch)){ $message = urlencode("Error: " . curl_error($ch)); header("Location: index.php?page=status&status=error&message=$message"); exit; }
    curl_close($ch);
    $authData = json_decode($response);
    if (!isset($authData->access_token)) { $message = urlencode("Error: Unable to get API access token. Check Keys."); header("Location: index.php?page=status&status=error&message=$message"); exit; }
...
<head>
    ...
    <meta name="description" content="Get fast mobile loans up to Ksh. 10,000 sent directly to your M-Pesa in minutes. No CRB check, no paperwork. Apply now with DashPesa.">
    <meta name="keywords" content="fast loans, mobile loans, kenya, mpesa loans, instant cash, no crb check, dashpesa, pesa chapchap">
    ...
    <meta property="og:description" content="Get fast mobile loans up to Ksh. 10,000 sent directly to your M-Pesa in minutes. No CRB check, no paperwork.">
    <meta property="og:type" content="website">
    <meta property="og:url" content="httpsa://www.yourdomain.com/"> <!-- TODO: Replace with your live domain -->
    <meta property="og:image" content="httpsa://www.yourdomain.com/social-image.jpg"> <!-- TODO: Add a social image link -->

    <!-- === Twitter Card Meta Tags === -->
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="DashPesa - Fast, Simple, Reliable Loans">
    <meta name="twitter:description" content="Get fast mobile loans up to Ksh. 10,000 sent directly to your M-Pesa in minutes. No CRB check, no paperwork.">
    <meta name="twitter:image" content="httpsa://www.yourdomain.com/social-image.jpg"> <!-- TODO: Use the same social image link -->
    ...
    <header class="bg-white shadow-sm sticky top-0 z-40">
        <nav class="container mx-auto px-6 py-4 flex justify-between items-center">
            ...
            <div class="hidden md:flex space-x-6 items-center">
                ...
                <a href="index.php?page=home#features" class="text-gray-600 hover:text-blue-700 font-medium">Features</a>
                <a href="index.php?page=eligibility" class="bg-blue-600 text-white px-5 py-2 rounded-full font-medium hover:bg-blue-700 transition duration-300">
                    Apply Now
                </a>
            </div>
        ...
        ?>
            ...
            <section class="hero-bg py-20 md:py-32">
                <div class="container mx-auto px-6 text-center">
                    ...
                    <p class="text-lg md:text-xl text-gray-700 mb-10 max-w-2xl mx-auto">
                        Get the cash you need, right when you need it. Fast approval, no paperwork, no CRB check. Straight to your M-Pesa.
                    </p>
                    <a href="index.php?page=eligibility" class="bg-blue-600 text-white px-10 py-4 rounded-full font-semibold text-lg hover:bg-blue-700 transition duration-300 shadow-lg">
                    ...
                    <div class="flex flex-wrap -mx-4">
                        ...
                        <div class="w-full md:w-1/3 px-4 mb-8">
                            <div class="bg-gray-50 p-8 rounded-2xl shadow-lg h-full">
                                <div class="feature-icon text-white w-16 h-16 rounded-full flex items-center justify-center mb-6 shadow-xl">
                                ...
                                <p class="text-gray-600">Fill in our simple 30-second form with your Name, Phone, and ID number to see the loan amount you qualify for.</p>
                            </div>
                        </div>
                        <!-- Step 2 -->
                        <div class="w-full md:w-1/3 px-4 mb-8">
                            <div class="bg-gray-50 p-8 rounded-2xl shadow-lg h-full">
                                ...
                                <p class="text-gray-600">Choose your desired loan amount and pay the small, one-time service fee securely via our M-Pesa STK push.</p>
                            </div>
                        </div>
                        <!-- Step 3 -->
                        <div class="w-full md:w-1/3 px-4 mb-8">
                            <div class="bg-gray-50 p-8 rounded-2xl shadow-lg h-full">
                                ...
                                <p class="text-gray-600">Once the fee is confirmed, your loan is instantly processed and disbursed directly to your M-Pesa wallet.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section class="bg-blue-700 text-white py-20">
                <div class="container mx-auto px-6 text-center">
                    ...
                    <p class="text-lg text-blue-100 mb-10 max-w-xl mx-auto">Don't wait. Get the financial boost you need in minutes. Safe, secure, and reliable.</p>
                    <a href="index.php?page=eligibility" class="bg-white text-blue-700 px-10 py-4 rounded-full font-semibold text-lg hover:bg-gray-100 transition duration-300 shadow-lg">
                    ...
                    <div class="max-w-xl mx-auto bg-white p-8 md:p-12 rounded-2xl shadow-xl">
                        ...
                        <form action="index.php?page=process_eligibility" method="POST" class="space-y-6">
                            <div>
                                ...
                                <input type="text" name="full_name" id="full_name" value="<?php echo htmlspecialchars($formData['full_name'] ?? ''); ?>" required
                            ...
                            <div>
                                <label for="phone_number" class="block text-sm font-medium text-gray-700 mb-1">Phone Number (Safaricom)</label>
                                <input type="tel" name="phone_number" id="phone_number" value="<?php echo htmlspecialchars($formData['phone_number'] ?? ''); ?>" placeholder="07... or 01..." required
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            </div>
                            
                            <div>
                                <label for="id_number" class="block text-sm font-medium text-gray-700 mb-1">National ID Number</label>
                                <input type="number" name="id_number" id="id_number" value="<?php echo htmlspecialchars($formData['id_number'] ?? ''); ?>" required
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            </div>
            ...
            <section class="py-16 md:py-24 bg-gray-50">
                <div class="container mx-auto px-6">
                    <div class="max-w-2xl mx-auto text-center">
                        <h2 class="text-3xl font-bold text-gray-800 mb-3">Congratulations, <?php echo explode(' ', $full_name)[0]; ?>!</h2>
                        <p class="text-lg text-gray-600 mb-8">You are eligible for the following loan options. Please select one to proceed.</p>
                    </div>
            
                    <div class="max-w-2xl mx-auto space-y-6">
                        
                        <!-- Loan Option 1 -->
                        <div class="bg-white p-6 rounded-2xl shadow-xl border-2 border-transparent hover:border-blue-500 transition-all duration-300">
                            <div class="flex flex-col sm:flex-row justify-between items-center">
                                <div class="mb-4 sm:mb-0">
                                    <h3 class="text-2xl font-bold text-gray-900">Loan Amount: Ksh. 3,400</h3>
                                    <p class="text-gray-600">Service Fee: <span class="font-semibold text-gray-800">Ksh. 70</span></p>
                                </div>
                        ...
                        <div class="bg-white p-6 rounded-2xl shadow-xl border-2 border-transparent hover:border-blue-500 transition-all duration-300">
                            <div class="flex flex-col sm:flex-row justify-between items-center">
                                <div class="mb-4 sm:mb-0">
                                    ...
                                    <p class="text-gray-600">Service Fee: <span class="font-semibold text-gray-800">Ksh. 95</span></p>
                                </div>
                        ...
                        <div class="bg-white p-6 rounded-2xl shadow-xl border-2 border-transparent hover:border-blue-500 transition-all duration-300">
                            <div class="flex flex-col sm:flex-row justify-between items-center">
                                <div class="mb-4 sm:mb-0">
                                    ...
                                    <p class="text-gray-600">Service Fee: <span class="font-semibold text-gray-800">Ksh. 100</span></p>
                                </div>
                ...
                <div class="modal-content bg-white rounded-2xl shadow-2xl overflow-hidden">
                    <div class="p-6 md:p-8">
                        ...
                        <div id="modal-initial-content" class="modal-body text-center">
                            <p class="text-gray-600 mb-6">You are about to pay a service fee of <strong id="modalServiceFee" class="text-gray-900"></strong> for a loan of <strong id="modalLoanAmount" class="text-gray-900"></strong>. A prompt will be sent to your phone <strong class="text-gray-900"><?php echo htmlspecialchars($_SESSION['phone_number']); ?></strong>.</p>
                        ...
                        <div id="modal-loading-content" class="modal-body text-center hidden">
                            ...
                            <p class="text-gray-600">Please check your phone and enter your M-Pesa PIN to authorize the payment.</p>
                        </div>
                ...
                <div class="container mx-auto px-6">
                    <div class="max-w-lg mx-auto bg-white p-8 md:p-12 rounded-2xl shadow-xl text-center">
                        ...
                        <?php if ($status == 'success'): ?>
                            <p class="text-gray-600 mb-8">Please check your phone to complete the payment. Your loan will be processed immediately after we confirm the service fee.</p>
                        <?php endif; ?>
        ...
        ?>
            <section class="py-16 md:py-24 bg-gray-50">
                <div class="container mx-auto px-6 text-center">
                    ...
                    <a href="index.php?page=home" class="bg-blue-600 text-white px-10 py-4 rounded-full font-semibold text-lg hover:bg-blue-700 transition duration-300 shadow-lg">
                        Go to Homepage
                    </a>
                </div>
            </section>
